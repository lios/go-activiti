...
func (processUtil *ProcessUtils) GetCurrentTask(taskId int) (model.FlowElement, error) {
...
func (processUtil *ProcessUtils) GetFlowElement(flowElementId string) (model.FlowElement, error) {
	flowM, ok := flowMap.Load(processUtil.ProcessId)
	if !ok {

	}
	flowElements, isOk := flowM.(map[string]model.FlowElement)
	if isOk {
